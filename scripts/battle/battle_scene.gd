...
const BattlePostBuildGuidanceTexture := preload("res://assets/generated/ui/battle_post_build_guidance_badge.png")
...
func _show_post_build_guidance() -> void:
	if _slot_buttons == null or _slot_layer == null:
		return
	_hide_post_build_guidance()
	for child: Node in _slot_layer.get_children():
		...
		if slot == null or bool(slot.get("occupied")):
			continue
		var guidance_size := Vector2(340, 162)
		var guidance_position := slot.position + Vector2(-118, 54) if slot.position.y < 240.0 else slot.position + Vector2(-124, -146)
		guidance_position.x = clampf(guidance_position.x, 12.0, 1280.0 - guidance_size.x - 12.0)
		guidance_position.y = clampf(guidance_position.y, 108.0, 720.0 - guidance_size.y - 14.0)

		var guidance: Control = Control.new()
		guidance.name = "BattlePostBuildGuidance"
		guidance.position = guidance_position
		guidance.size = guidance_size
		guidance.mouse_filter = Control.MOUSE_FILTER_IGNORE
		guidance.z_index = 23
		guidance.set_meta("image2_post_build_guidance", true)
		guidance.set_meta("built_tower_count", towers.size())
		_slot_buttons.add_child(guidance)

		var badge: TextureRect = _hud_texture_rect("BattlePostBuildGuidanceBadge", BattlePostBuildGuidanceTexture, Vector2.ZERO, guidance_size)
		badge.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
		badge.z_index = 1
		guidance.add_child(badge)

		var label: Label = _hud_label("继续补防")
		label.name = "BattlePostBuildGuidanceLabel"
		label.position = Vector2(112, 62)
		label.size = Vector2(132, 38)
		label.add_theme_font_size_override("font_size", 21)
		label.add_theme_color_override("font_color", Color(0.36, 0.16, 0.05))
		label.add_theme_color_override("font_outline_color", Color(1.0, 0.92, 0.64, 0.90))
		label.add_theme_constant_override("outline_size", 4)
		...
		guidance.pivot_offset = guidance.size * 0.5
		guidance.scale = Vector2(0.76, 0.76)
		guidance.modulate = Color(1.0, 1.0, 1.0, 0.0)
		var guidance_ref: WeakRef = weakref(guidance)
		var intro: Tween = guidance.create_tween()
		intro.set_parallel(true)
		...
		intro.tween_callback(func() -> void:
			var resolved: Object = guidance_ref.get_ref()
			if resolved is Control:
				_start_post_build_guidance_float(resolved as Control)
		).set_delay(0.22)
		return


func _start_post_build_guidance_float(guidance: Control) -> void:
	if guidance == null or not is_instance_valid(guidance):
		return
	var base_position: Vector2 = guidance.position
	var tween: Tween = guidance.create_tween()
	tween.set_loops()
	tween.tween_property(guidance, "position:y", base_position.y - 7.0, 0.72).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(guidance, "position:y", base_position.y, 0.72).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)


func _hide_post_build_guidance() -> void:
	if _slot_buttons == null:
		return
	var guidance: Control = _slot_buttons.find_child("BattlePostBuildGuidance", true, false) as Control
	if guidance != null:
		guidance.queue_free()
...
func _upgrade_tower_from_overlay(tower: Node2D, feedback_target: Control) -> void:
	...
	coins -= upgrade_cost
	_update_hud()
	_show_tower_upgrade_spend_feedback(upgrade_cost, tower.global_position)
...
func _show_tower_upgrade_spend_feedback(cost: int, world_anchor: Vector2) -> void:
	if _hud == null or _coins_label == null or cost <= 0:
		return
	_tower_upgrade_spend_feedback_index += 1
	_coins_label.set_meta("image2_tower_upgrade_spend_source", true)

	var chip_size := Vector2(72, 72)
	var start_center: Vector2 = _coins_label.position + _coins_label.size * 0.5 + Vector2(18.0, 0.0)
	var target_center := Vector2(
		clampf(world_anchor.x, 96.0, 1120.0),
		clampf(world_anchor.y - 44.0, 116.0, 604.0)
	)
	var arc_center := Vector2(
		(start_center.x + target_center.x) * 0.5,
		clampf(min(start_center.y, target_center.y) + 96.0, 88.0, 340.0)
	)

	var chip: TextureRect = _hud_texture_rect("TowerUpgradeSpendFish%d" % _tower_upgrade_spend_feedback_index, TowerUpgradeSpendFishChipTexture, start_center - chip_size * 0.5, chip_size)
	chip.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	chip.z_index = 92
	chip.process_mode = Node.PROCESS_MODE_ALWAYS
	chip.pivot_offset = chip.size * 0.5
	...
	chip.modulate = Color(1.0, 1.0, 1.0, 0.0)
	chip.set_meta("image2_tower_upgrade_spend_feedback", true)
	chip.set_meta("cost", cost)
	_hud.add_child(chip)

	var amount: Label = _hud_label("-%d" % cost)
	amount.name = "TowerUpgradeSpendAmountLabel"
	amount.position = Vector2(8, 50)
	amount.size = Vector2(56, 22)
	amount.add_theme_font_size_override("font_size", 14)
	amount.add_theme_color_override("font_color", Color(0.64, 0.12, 0.05))
	amount.add_theme_color_override("font_outline_color", Color(1.0, 0.90, 0.58, 0.94))
	amount.add_theme_constant_override("outline_size", 3)
	amount.mouse_filter = Control.MOUSE_FILTER_IGNORE
	amount.clip_text = true
	chip.add_child(amount)
	...
	var coins_ref: WeakRef = weakref(_coins_label)
	var move_tween: Tween = chip.create_tween()
	move_tween.tween_property(chip, "position", arc_center - chip_size * 0.5, 0.24).set_delay(0.03).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	move_tween.tween_property(chip, "position", target_center - chip_size * 0.5, 0.32).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN)

	var visual_tween: Tween = chip.create_tween()
	visual_tween.set_parallel(true)
	visual_tween.tween_property(chip, "modulate:a", 1.0, 0.06).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	visual_tween.tween_property(chip, "scale", Vector2(0.84, 0.84), 0.10).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	visual_tween.tween_property(chip, "scale", Vector2(0.42, 0.42), 0.28).set_delay(0.34).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	visual_tween.tween_property(chip, "rotation_degrees", -14.0, 0.54).set_delay(0.04).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	visual_tween.tween_property(chip, "modulate:a", 0.0, 0.12).set_delay(0.64).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	visual_tween.tween_callback(func() -> void:
		var source: Object = coins_ref.get_ref()
		if source is Control:
			_animate_control_scale(source as Control, 0.90, 0.07)
	).set_delay(0.04)
	visual_tween.tween_callback(func() -> void:
		var source: Object = coins_ref.get_ref()
		if source is Control:
			_animate_control_scale(source as Control, 1.10, 0.08)
	).set_delay(0.58)
	visual_tween.tween_callback(func() -> void:
		var resolved: Object = chip_ref.get_ref()
		if resolved is Node:
...
func _show_pause_restart_feedback() -> void:
	if _hud == null:
		return
	var existing: Node = _hud.find_child("PauseRestartFeedback", true, false)
	if existing != null:
		existing.queue_free()

	var feedback_size := Vector2(380, 210)
	var feedback: TextureRect = _hud_texture_rect("PauseRestartFeedback", PauseRestartFeedbackBadgeTexture, Vector2(450, 132), feedback_size)
	feedback.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	feedback.z_index = 94
	feedback.process_mode = Node.PROCESS_MODE_ALWAYS
	feedback.pivot_offset = feedback.size * 0.5
	feedback.scale = Vector2(0.62, 0.62)
	feedback.modulate = Color(1.0, 1.0, 1.0, 0.0)
	feedback.set_meta("image2_pause_restart_feedback", true)
	_hud.add_child(feedback)

	var label: Label = _hud_label("重新开局")
	label.name = "PauseRestartFeedbackLabel"
	label.position = Vector2(148, 84)
	label.size = Vector2(196, 50)
	label.add_theme_font_size_override("font_size", 32)
	label.add_theme_color_override("font_color", Color(0.40, 0.18, 0.05))
	label.add_theme_color_override("font_outline_color", Color(1.0, 0.92, 0.62, 0.94))
	label.add_theme_constant_override("outline_size", 4)
	label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	label.clip_text = true
	feedback.add_child(label)

	var sub_label: Label = _hud_label("守卫开始")
	sub_label.name = "PauseRestartFeedbackSubLabel"
	sub_label.position = Vector2(156, 128)
	sub_label.size = Vector2(172, 34)
	sub_label.add_theme_font_size_override("font_size", 18)
	sub_label.add_theme_color_override("font_color", Color(0.48, 0.25, 0.08))
	sub_label.add_theme_constant_override("outline_size", 2)
	sub_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	feedback.add_child(sub_label)
	...
	tween.tween_property(feedback, "modulate:a", 1.0, 0.10).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	tween.tween_property(feedback, "scale", Vector2.ONE, 0.20).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween.tween_property(feedback, "position:y", feedback.position.y - 10.0, 0.52).set_delay(0.22).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(feedback, "position:y", feedback.position.y - 3.0, 0.48).set_delay(0.74).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(feedback, "modulate:a", 0.0, 0.26).set_delay(1.32).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	tween.tween_callback(func() -> void:
		var resolved: Object = feedback_ref.get_ref()
		if resolved is Node:
			(resolved as Node).queue_free()
