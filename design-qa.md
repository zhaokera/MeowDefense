...
**Image2 Daily Task Progress Guidance**
- Added project-bound Image2-derived assets `/Users/zhaok/cat/assets/generated/ui/daily_task_progress_guidance_design_reference.png`, `/Users/zhaok/cat/assets/generated/ui/daily_task_progress_guidance_burst_source.png`, `/Users/zhaok/cat/assets/generated/ui/daily_task_progress_guidance_burst.png`, `/Users/zhaok/cat/assets/generated/ui/daily_task_progress_level_guidance_design_reference.png`, `/Users/zhaok/cat/assets/generated/ui/daily_task_progress_level_guidance_badge_source.png`, and `/Users/zhaok/cat/assets/generated/ui/daily_task_progress_level_guidance_badge.png`. A fresh Image2 direction was requested, but the local built-in image tool again did not expose a project-copyable output, so the final referenced assets were assembled from existing project-bound Image2 daily-task, achievement-progress, reward, and level-select art.
- Unfinished daily task rows now expose `DailyTaskFirstClearProgressButton` style transparent progress hit areas over the Image2 progress chip, while the disabled claim button stays disabled and cannot grant fish early.
- Pressing an unfinished task opens `DailyTaskProgressGuidanceOverlay`, a full-screen Image2 guidance design with runtime task title, requirement, reward, progress, and `去关卡` action copy.
- Pressing `GoLevelsFromDailyTaskProgressButton` preserves the shared Image2 overlay exit animation, opens level select, and shows `DailyTaskProgressLevelGuidance`, a non-blocking Image2 badge with dynamic `今日任务` / `通关领取` copy while leaving `StartLevel1Button` tappable.
...
**Image2 Daily Task Claim Shop Guidance**
- Added project-bound Image2-derived assets `/Users/zhaok/cat/assets/generated/ui/daily_task_claim_shop_guidance_design_reference.png`, `/Users/zhaok/cat/assets/generated/ui/daily_task_claim_shop_guidance_badge_source.png`, and `/Users/zhaok/cat/assets/generated/ui/daily_task_claim_shop_guidance_badge.png`, assembled from existing Image2 daily-reward shop guidance art after the fresh Image2 task-claim shop concept was not exposed as a project-copyable file by the local Codex image tool.
- Ordinary daily task claims now detect when the reward crosses the 25-fish yarn-trap price and show `DailyTaskClaimShopGuidance`, a non-blocking Image2 badge with dynamic `去商店` / `买毛线` copy on the task reward overlay.
- Small task rewards that remain below the yarn price keep the plain claim reward overlay, and shortage-driven claims still use the separate `DailyTaskShopReturnGuidance` path.
- Pressing `DailyTaskClaimShopButton` preserves the shared Image2 reward-overlay exit animation, disables the route button during transition, closes `DailyTaskOverlay`, opens `ShopOverlay`, and marks `BuyShopYarnTrapKitButton` plus `ShopYarnTrapKitBuyButtonFrame` with `image2_daily_task_claim_shop_target`.
