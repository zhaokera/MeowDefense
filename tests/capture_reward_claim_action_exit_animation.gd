...
const OUT_PATH := "/Users/zhaok/cat/artifacts/reward_claim_action_exit_animation.png"
const TEST_SAVE_PATH := "user://meow_defense_reward_claim_action_exit_animation_capture_save.json"
...
func _capture() -> void:
	...
	if overlay == null or claim_button == null:
		push_error("RewardOverlay or ClaimRewardButton missing")
		quit(1)
		return
	...
	if not bool(overlay.get_meta("image2_overlay_exit_animation", false)):
		push_error("RewardOverlay should mark Image2 claim exit animation")
		Engine.time_scale = 1.0
		quit(1)
		return
	if not is_instance_valid(overlay):
		push_error("RewardOverlay should remain visible during slowed claim exit animation")
		Engine.time_scale = 1.0
		quit(1)
		return
	if instance.find_child("DailyRewardClaimSuccessOverlay", true, false) != null:
		push_error("DailyRewardClaimSuccessOverlay should wait for claim exit animation")
		Engine.time_scale = 1.0
		quit(1)
		return
