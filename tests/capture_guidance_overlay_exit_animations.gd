...
const TEST_SAVE_PATH := "user://meow_defense_guidance_overlay_exit_animation_capture_save.json"
...
func _capture() -> void:
	...
	if not await _capture_locked_level_exit(instance):
		return
	if not await _capture_energy_empty_exit(instance):
		return
	if not await _capture_daily_task_exit(instance):
		return
...
func _capture_locked_level_exit(instance: Node) -> bool:
	...
	if locked_info == null:
		push_error("LockedLevel2InfoButton missing")
		quit(1)
		return false
	...
	return await _capture_overlay_exit(
		instance,
		"LockedLevelFeedbackOverlay",
		"CloseLockedLevelFeedbackButton",
		"locked_level_feedback_exit_animation.png"
	)
...
func _capture_energy_empty_exit(instance: Node) -> bool:
	...
	return await _capture_overlay_exit(
		instance,
		"EnergyEmptyOverlay",
		"CloseEnergyEmptyButton",
		"energy_empty_overlay_exit_animation.png"
	)
...
func _capture_daily_task_exit(instance: Node) -> bool:
	...
	return await _capture_overlay_exit(
		instance,
		"DailyTaskOverlay",
		"CloseDailyTaskButton",
		"daily_task_overlay_exit_animation.png"
	)
