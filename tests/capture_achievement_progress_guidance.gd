...
const OUT_PATH := "/Users/zhaok/cat/artifacts/achievement_progress_guidance.png"
const TEST_SAVE_PATH := "user://meow_defense_achievement_progress_guidance_capture_save.json"
...
func _capture() -> void:
	...
	if error != OK:
		push_error("failed to save achievement progress guidance screenshot: %s" % error)
		quit(1)
		return
...
func _clear_save_file() -> void:
	if FileAccess.file_exists(TEST_SAVE_PATH):
		...
		if error != OK:
			push_error("failed to clear achievement progress guidance capture save: %s" % error)
