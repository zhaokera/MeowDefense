...
const OUT_PATH := "/Users/zhaok/cat/artifacts/result_campaign_clear_guidance.png"
const TEST_SAVE_PATH := "user://meow_defense_result_campaign_clear_guidance_capture_save.json"
...
func _capture() -> void:
	...
	var sub_label: Label = instance.find_child("ResultAchievementClaimGuidanceSubLabel", true, false) as Label
	if sub_label == null or not sub_label.text.contains("连续推进"):
		push_error("ResultAchievementClaimGuidanceSubLabel missing campaign-clear copy")
		quit(1)
		return
	...
	if result != OK:
		push_error("failed to save %s: %s" % [OUT_PATH, result])
		quit(1)
		return
...
func _clear_save_file() -> void:
	if FileAccess.file_exists(TEST_SAVE_PATH):
		...
		if error != OK:
			push_error("failed to clear result campaign clear capture save: %s" % error)
