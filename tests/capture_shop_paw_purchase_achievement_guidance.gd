...
const OUT_PATH := "/Users/zhaok/cat/artifacts/shop_paw_purchase_achievement_guidance.png"
const TEST_SAVE_PATH := "user://meow_defense_shop_paw_purchase_achievement_guidance_capture_save.json"
...
func _capture() -> void:
	...
	await _wait_until_exists(instance, "ShopPawPurchaseAchievementGuidance")
	for _frame: int in range(36):
		await process_frame
	await RenderingServer.frame_post_draw

	var route_button: Button = instance.find_child("ShopPawPurchaseAchievementButton", true, false) as Button
	if route_button == null:
		push_error("ShopPawPurchaseAchievementButton missing")
		quit(1)
		return
	var label: Label = instance.find_child("ShopPawPurchaseAchievementLabel", true, false) as Label
	if label == null or not label.text.contains("成就"):
		push_error("ShopPawPurchaseAchievementLabel missing expected copy")
		quit(1)
		return
...
func _clear_save_file() -> void:
	if FileAccess.file_exists(TEST_SAVE_PATH):
		...
		if error != OK:
			push_error("failed to clear shop paw purchase achievement guidance capture save: %s" % error)
